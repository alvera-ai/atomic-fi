defmodule AlveraPhoenixTemplateServer.OpenApiSchema do
  @moduledoc """
  ...
  defmodule TenantRequest do
    ...
    OpenApiSpex.schema(%{
      title: "TenantRequest",
      description: "Request body for creating/updating tenants",
      type: :object,
      properties: %{
        name: %Schema{type: :string, description: "Tenant name"},
        slug: %Schema{type: :string, description: "URL-safe identifier"},
        status: %Schema{type: :string, enum: ["active", "suspended", "inactive"]},
        tenant_type: %Schema{type: :string, enum: ["platform", "standard"]},
        metadata: %Schema{type: :object, description: "Tenant metadata"}
      },
      required: [:name, :status, :tenant_type]
    })
  end
  ...
  defmodule TenantResponse do
    ...
    OpenApiSpex.schema(%{
      ...
      required: [:id, :name, :status, :tenant_type]
    })
  end

  ## List Responses (using deflistresponse macro)
  require AlveraPhoenixTemplateServer.OpenApiSchemaHelpers
  alias AlveraPhoenixTemplateServer.OpenApiSchemaHelpers
