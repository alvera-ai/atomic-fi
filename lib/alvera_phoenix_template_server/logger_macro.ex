defmodule AlveraPhoenixTemplateServer.LoggerMacro do
  @moduledoc """
    ...
    quote do
      def unquote(call) do
                ...
                [
                  unquote_splicing(
                    for {field, var} <- log_metadata do
                      quote do
                        {unquote(field),
                         AlveraPhoenixTemplateServer.LoggerMacro.extract_loggable_value(
                           unquote(var)
                         )}
                      end
                    end
        ...
        try do
          ...
          status =
            AlveraPhoenixTemplateServer.LoggerMacro.extract_result_status(result)

          target_object_id =
            AlveraPhoenixTemplateServer.LoggerMacro.extract_target_object_id(result)
