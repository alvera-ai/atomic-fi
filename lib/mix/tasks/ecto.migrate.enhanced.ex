defmodule Mix.Tasks.Ecto.Migrate.Enhanced do
  ...
  1. Reads migration paths from Application config (:alvera_phoenix_template_server, :migration_paths)
  ...
  def run(args) do
    ...
    migration_paths_config =
      Application.get_env(:alvera_phoenix_template_server, :migration_paths, %{})
  ...
  defp do_migrate_path(repo, path, args) do
    ...
    repo_config = Application.get_env(:alvera_phoenix_template_server, repo, [])

    app = Keyword.get(repo_config, :otp_app, :alvera_phoenix_template_server)
