...
```
lib/payment_compliance_platform_api/
...
In your `lib/payment_compliance_platform_web/router.ex`:

```elixir
defmodule PaymentCompliancePlatformWeb.Router do
  use PaymentCompliancePlatformWeb, :router
  use PaymentCompliancePlatformApi.Routes  # Add this line
...
- `lib/payment_compliance_platform/plugs/api_auth.ex`
...
OpenAPI schemas are automatically generated from your Ecto schemas when using `PaymentCompliancePlatform.Schema`.
...
defmodule PaymentCompliancePlatformApi.UserControllerTest do
  use PaymentCompliancePlatformWeb.ConnCase
  ...
  test "lists users", %{conn: conn} do
    ...
    assert_schema(response, "UserListResponse", PaymentCompliancePlatformApi.ApiSpec.spec())
  end
end
```
