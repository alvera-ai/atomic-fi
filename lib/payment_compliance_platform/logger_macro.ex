defmodule PaymentCompliancePlatform.LoggerMacro do
  @moduledoc """
    ...
    quote do
      def unquote(call) do
                ...
                [
                  unquote_splicing(
                    for {field, var} <- log_metadata do
                      quote do
                        {unquote(field),
                         PaymentCompliancePlatform.LoggerMacro.extract_loggable_value(
                           unquote(var)
                         )}
                      end
                    end
        ...
        try do
          ...
          status =
            PaymentCompliancePlatform.LoggerMacro.extract_result_status(result)

          target_object_id =
            PaymentCompliancePlatform.LoggerMacro.extract_target_object_id(result)
