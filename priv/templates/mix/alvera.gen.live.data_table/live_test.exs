defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>LiveTest do
  ...
  describe "Index" do
    ...
    test "toggles AI search", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"<%= schema.route_prefix %>")

      refute has_element?(index_live, "#ai-search-container")

      index_live |> element("button", "AI Search") |> render_click()

      assert has_element?(index_live, "#ai-search-container")
    end
  end
