...
- `lib/atomic_fi/<context>/<schema>.ex` - Schema with TypedEctoSchema + OpenAPI annotations
- `lib/atomic_fi/<context>.ex` - Context module with complete CRUD
- `priv/repo/migrations/TIMESTAMP_create_<plural>.exs` - Migration with column comments
- `test/atomic_fi/<context>_test.exs` - Complete tests with @moduletag :refactored
...
defmodule AtomicFi.Accounts.User do
  use AtomicFi.Schema  # Includes TypedEctoSchema + ExOpenApiUtils
  ...
  * `updated_at` - Timestamp when user was last updated
  ...
  typed_schema "users" do
    ...
    belongs_to :tenant, AtomicFi.TenantContext.Tenant
...
```elixir
defmodule AtomicFi.Accounts do
  import Ecto.Query
  alias AtomicFi.Repo
...
```elixir
defmodule AtomicFi.AccountsTest do
